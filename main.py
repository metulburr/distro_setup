...
def sfml_install():
    ...
    p.wait()
    p = subprocess.Popen('sudo cp -r * /usr/lib'.split())
    p.wait()
    p = subprocess.Popen('sudo cp -r share/SFML /usr/share'.split())
    p.wait()
    
    print("command to compile in IDE's:")
    print(r'''g++ -std=c++11 -Wall -o "%e" "%f" -lsfml-audio -lsfml-graphics -lsfml-window -lsfml-system &&"./%e"''')
    time.sleep(1)
    
def update_gcc():
    #update ubuntu's gcc, g++ from normally 4.6 to 4.8
    cmds = [
